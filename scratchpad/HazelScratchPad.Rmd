...
projects_factored <- projects %>%
  ...
  mutate(funds_needed =round((total_price_excluding_optional_support -
                          total_donations), 0)) %>%
  left_join(project_ids, by = c("_projectid" = "projectid")) %>%
  ...
  select(pid, state_id, metro_id, school_charter,
...
projects_factored[projects_factored == "t"] <- 1
projects_factored[projects_factored == "f"] <- 0

# Convert the projects_factored data frame into a matrix
projects_mat <- as.matrix(projects_factored[,-1:23])
rownames(projects_mat) <- as.numeric(projects_factored$pid)

# Make sure the factors were converted to nummeric
mode(projects_mat) <- "numeric" 

# Do the PCA Analysis
pca <- prcomp(projects_mat)
print(pca)
