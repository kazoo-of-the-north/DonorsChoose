...
train_set <- slice(projects.factored, inTrain$Resample1)
test_set <- slice(projects.factored, -inTrain$Resample1)
...
plot(X.pca, type = "l")
summary(X.pca)$importance[,1:15]

plot(X.pca$sdev)


model <- glm(train_set$fully_funded ~ X.pca$x[,1:8], family=binomial)
summary(model)

test.p <- predict(X.pca, newdata = X.test)
pred <- predict(model, newdata = data.frame(test.p), type = "response")
