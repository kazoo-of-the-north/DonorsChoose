...
# Convert the projects_factored data frame into a matrix
projects.matrix <- as.matrix(projects.factored[,2:22])
rownames(projects.matrix) <- projects.factored$pid

# Make sure the factors were converted to nummeric
mode(projects.matrix) <- "numeric" 
...
# and scaled
projects.pca <- prcomp(projects.matrix, scale=TRUE)

plot(projects.pca, type = "l")
summary(projects.pca)$importance[,1:15]

plot(projects.pca$sdev)

projects.pca2 <- prcomp(projects.matrix, scale=TRUE, center=FALSE)

plot(projects.pca2, type = "l")
summary(projects.pca2)$importance[,1:15]

plot(projects.pca2$sdev)
...
# Rank words based off of percentage of occurance in completed and
# essays
essays_word_rank <- essays_word_freq %>%
...
words_unmatched <- essays_word_rank %>%
  filter(abs(pct_rank_completed-pct_rank_expired) > 0.75)

# Get the sentiments associated with these words
words_unmatched_sent <- words_unmatched %>%
    left_join(nrc, by = c("essay_words" = "word")) %>%
    group_by(sentiment) %>%
    mutate(completed_freq = sum(completed_freq),
           expired_freq= sum(expired_freq)) %>%
    ungroup()

# Print out the sentiments for the words that appear often in
# essays for "completed" projects, but not in essays for 
# "expired" projects
print(words_unmatched_sent)
