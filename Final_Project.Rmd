...
The c++ application made with the source code above was disadvantageous in that it would only run on Macintosh computers, used by 2 out of 3 group members. So this solution was set aside, in favor of the solution written in R, with source found in <a href="#dataloader_in_r">Appendix C</a>, so everyone could run the same dataloader.
...
# TODO: eval needs to be set to true prior to handing in
...
```{r win loss word breakdowns, echo=FALSE}
