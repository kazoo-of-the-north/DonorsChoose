---
title: "Donors Choose Funding Analysis"
author: "Steve Henle, Hazel John, and Jim Schlough"
date: "April 28, 2016"
output:
  html_document:
    toc: true
...
```
<a id="displayText" href="javascript:toggle(99);">Show or hide to see compile time setup</a>
  <div id="toggleText99" style="display: none">  
```{r compile time stopwatch start, echo=FALSE}
# looking at compile times: 
if (exists("startTime")) {rm(startTime );}
if (exists("stopTime")) {rm(stopTime);}
startTime<-timestamp()
startTime
...
Hazel works with her marketing department on content marketing, personalization etc. She is interested in understanding what motivates prospects to become customers. In this case we analyze factors that encourage prospects to turn into benefactors.
...
The Donors Choose platform has been held up as a model for transparency, and has encouraged both informal and formal study of its data. A little more than 13 years of static data have been made available to the data analyzing public. There is also an API to show a live, tickertape style feed of projects.

There was the [2014 KDD Cup](https://www.kaggle.com/c/kdd-cup-2014-predicting-excitement-at-donors-choose) to predict the excitement surrounding a given project proposal on Donors Choose. KDD stands for _Knowledge, Discovery, and Data Mining_. This contest was given the name: "Predict funding requests that deserve an A+"

[Here](http://jcberk.com/donorschoose/) is one of the better examples of research done surrounding the Donors Choose data, by JC Berk, _et al_.
...
This data and data documentation can be found  [here](http://data.donorschoose.org/open-data/overview/).

Multiple data sets are available for download including project, donations, resources and essay data sets. The database [schema](http://i0.wp.com/data.donorschoose.org/wp-content/uploads/2013/12/DonorsChoose-org-data-schema-04012011.png) and data descriptions are available as well from the Donors Choose website.

<a id="displayText" href="javascript:toggle(1);">Schema Diagram</a>
...
<a id="displayText" href="javascript:toggle(3);">Code: Data Wrangling</a>
...
<a id="displayText" href="javascript:toggle(4);">Code: Data Upload from Disk</a>
...
<a id="displayText" href="javascript:toggle(5);">Code: Data Preparation</a>
...
Data is nowhere near normal, looks like logistic analysis of funded vs non-funded make much more sense.
...
We started by taking a quick look at percenge funding of projects across all states.

<a id="displayText" href="javascript:toggle(6);">Code: Distribution Across US </a>
  <div id="toggleText6" style="display: none">

```{r Demographics US Map, warning=FALSE}

#load us map data
all_states <- map_data("state")
#plot all states with ggplot
map <- projects %>% ggplot()
map <- map + geom_polygon( data=all_states, 
...
We then explored the available demographic factors further.

<a id="displayText" href="javascript:toggle(7);">Code: Analysis of Demographics </a>
  <div id="toggleText7" style="display: none">
  
```{r poverty level graph, warning=FALSE}
#poverty level graph
...
urban <- projects %>% group_by(school_metro, funding_status) %>% 
  ...
  ungroup %>% 
  filter(funding_status == "completed") %>%
  filter(school_metro == "rural" | school_metro == "suburban" |
           school_metro == "urban") %>%
  ggplot(aes(school_metro, percent, fill = school_metro)) +
...
<a id="displayText" href="javascript:toggle(8);">Code: Analysis of Teacher Qualification </a>
  <div id="toggleText8" style="display: none">
  
```{r Teacher Attributes,warning=FALSE}
...
<a id="displayText" href="javascript:toggle(9);">Code: Analysis of School Attributes </a>
  <div id="toggleText9" style="display: none">
  
```{r School Attributes, warning=FALSE}
#charter school
...
Again we can see several small effects, but all of these variables only affect a very small percentage of schools.
...
<a id="displayText" href="javascript:toggle(10);">Code: Analysis of Classroom Attributes </a>
  <div id="toggleText10" style="display: none">
  
```{r Classroom Attributes, warning=FALSE}
...
<a id="displayText" href="javascript:toggle(11);">Code: Analysis of Financial Attributes </a>
  <div id="toggleText11" style="display: none">
  
```{r Financial Attributes, warning=FALSE}
...
```{r histogram showing cost of all proposals,warning=FALSE}
...
<a id="displayText" href="javascript:toggle(12);">Code: Distribution of lower cose projects </a>
...
<a id="displayText" href="javascript:toggle(13);">Code: Model Fitting (1) </a>
...
<a id="displayText" href="javascript:toggle(14);">Code: Model Fitting (2) </a>
...
<a id="displayText" href="javascript:toggle(15);">Code: Model Fitting (3) </a>
...
<a id="displayText" href="javascript:toggle(16);">Code: Final Model </a>
...
<a id="displayText" href="javascript:toggle(17);">Code: Sentiment Analysis</a>
...
<a id="displayText" href="javascript:toggle(18);">Code: Word Analysis</a>
...
<a id="displayText" href="javascript:toggle(19);">Code: Success Words Analysis</a>
...
We were able to identify "surprise" and "anticipation" as sentiments associated with words that show up more in essays associated with "completed" projects as opposed to "expired" projects.
...
<a id="displayText" href="javascript:toggle(20);">Code: Word Count Analysis</a>
...
###Aspects of Essays
We looked at sentiments evoked by the word choise of essays in projects that were both completed and expired, and also the length of the number of words within the essays. No great differences were found within the word choice and essay lengths.<br>

####Essay Word Selection Sentiments
...
These involve factors regarding school and classroom properties and attributes. These would include year round schools, and other special school classifications.<br>
...
2. R packages used in this project: <br>
...
<br>Baptiste Auguie, Anton Antonov <br>
+ <b>lubridate  </b>  <br>
[lubridate](https://cran.r-project.org/web/packages/lubridate/index.html)
<br>Garrett Grolemund [aut], Vitalie Spinu, Hadley Wickham , Jan Lyttle, Imanuel Constigan, Jason Law [ctb], Doug Mitarotonda [ctb], Joseph Larmarange , Jonathan Boiser, Chel Hee Lee [ctb]<br>
+ <b>maps </b>  <br> 
[maps](https://cran.r-project.org/web/packages/maps/maps.pdf)
<br>Original S code by Richard A. Becker and Allan R. Wilks.
...
<br>Hadley Wickham, Romain Francois, R Core Team (Date time code adapted from R), RStudio <br>
+ <b>reshape2 </b>  <br> 
[reshape2](https://cran.r-project.org/web/packages/reshape2/index.html)
<br>Hadley Wickham<br>
+ <b>stringr  </b>  <br>
[stringr](https://cran.r-project.org/web/packages/stringr/index.html)<br>Hadley Wickham <br>
+ <b>tidyr</b>  <br>
[tidyr](https://cran.r-project.org/web/packages/tidyr/index.html)<br>Hadley Wickham<br>
+ <b>tidytext</b>  <br>
[tidytext](https://cran.rstudio.com/web/packages/tidytext/)<br>Gabriela De Queiroz [ctb], David Robinson [aut], Julia Silge [aut, cre]
<br>
+ <b>wordcloud </b>  <br>
[wordcloud](https://cran.r-project.org/web/packages/wordcloud/wordcloud.pdf)
<br>Ian Fellows<br>

<a id="displayText" href="javascript:toggle(100);">Show or hide to see compile time</a>
